...
{
    public interface INotifySauceTestResults
    {
        void Pass(string sessionId, string testName);
        void Fail(string sessionId, string testName);
    }

    public class SauceSeleniumTestResult : INotifySauceTestResults
    {
        private SauceREST _sauceRest;
        private const string _sessionOutputFormat = "SauceOnDemandSessionID={0} job-name={1}";

        public SauceSeleniumTestResult(SauceREST sauceRest)
        {
            _sauceRest = sauceRest;
        }

        public void Pass(string sessionId, string testName)
        {
            Console.WriteLine(_sessionOutputFormat, sessionId, testName);
            _sauceRest.jobPassed(sessionId);
            _sauceRest.downloadVideo(sessionId, "c:\\Videos\\");
        }

        public void Fail(string sessionId, string testName)
        {
            Console.WriteLine(_sessionOutputFormat, sessionId, testName);
            _sauceRest.jobFailed(sessionId);
        }
    }
    ...
    public sealed class SauceREST
    {
        ...
        public SauceREST(SauceConfiguration configuration)
        {
        ...
        private void downloadFile(string jobId, string location, Uri restEndpoint)
        {
            ...
            Console.WriteLine(restEndpoint.AbsoluteUri);

            Thread.Sleep(5000);

            using (var client = new WebClient())
            {
                string credentials = Convert.ToBase64String(Encoding.ASCII.GetBytes(username + ":" + accessKey));
                client.Headers[HttpRequestHeader.Authorization] = string.Format("Basic {0}", credentials);
                client.DownloadFile(restEndpoint, string.Concat(jobId, DateTime.Now.ToString(DATE_FORMAT), ".flv"));
            }
        }
